...
Route::get('bigsitemap', function()
{
    ...
    {
      $sitemap->add(URL::to('/category/' . $cate->category), date('Y-m-d H:i:s'), '0.9', 'monthly');
      $counter++;
    }
    ...
    {
      $sitemap->add(URL::to('/brand/' . $brand->url_slug), date('Y-m-d H:i:s'), '0.9', 'monthly');
      $counter++;
    }
    ...
    foreach ($posts as $post)
    {
        if ($counter == 50000)
        {
            ...
            $sitemap->addSitemap('sitemap-'.$sitemapCounter.'.xml');
